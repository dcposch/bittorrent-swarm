...
WebConn.prototype.httpRequest = function (pieceIndex, offset, length, cb) {
  ...
  if (files.length <= 1) {
    ...
    var requestedFiles = files.filter(function (file) {
      return file.offset <= rangeStart && (file.offset + file.length) > rangeStart
    })
    if (requestedFiles.length < 1) return cb(new Error('Could not find file corresponnding to web seed range request'))
    if (requestedFiles.length > 1) return cb(new Error('Web seed range request overlaps multiple files in torrent'))
    var requestedFile = requestedFiles[0]
    var fileEnd = requestedFile.offset + requestedFile.length - 1

    url = self.url +
      (self.url[self.url.length - 1] === '/' ? '' : '/') +
      requestedFile.path
    start = rangeStart - requestedFile.offset
    end = isNaN(rangeEnd) ? fileEnd : Math.min(fileEnd, rangeEnd - requestedFile.offset)
  }
